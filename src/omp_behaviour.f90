...
integer :: kcell
integer :: k, idc, site(3), indx(2), ctype, stype
logical :: cognate
...
elseif (indx(2) == kcell) then
    ...
    write(*,*) 'ERROR: Tcell_death: cell not at site: ',kcell,site,indx
    stop
endif
...
if (p2%avidity /= p1%avidity) then
    ...
    ! replace avidity generated by create_Tcell with parent avidity
    p2%avidity = p1%avidity
!    i = (p2%avidity -avidity_min)/avidity_step + 1.5
!    i = max(i,1)
!    i = min(i,avidity_nlevels)
!    adist(i+1) = adist(i+1) + 1
endif
!write(*,'(a,8i6)') 'adist: ',adist
...
!if (avid_debug) then
!    if (.not.associated(cell%cptr)) then
!        write(nfout,*) '(2) cptr not associated: ',kcell
!    else
!        write(nfout,'(a,i7,2f8.4)') 'create_Tcell (2): ',kcell, cell%cptr%avidity
!    endif
!endif
